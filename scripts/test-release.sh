...
export GITHUB_REPOSITORY=example/ahx DEFAULT_BRANCH=main CARGO_REGISTRY_TOKEN=test-only
...
MANIFEST
# Independent workspaces must retain locked builds after every version bump.
for entry in fuzz tests/no-std; do
	mkdir -p "$entry/src"
	: >"$entry/src/lib.rs"
	cat >"$entry/Cargo.toml" <<MANIFEST
[package]
name = "ahx-$(basename "$entry")-check"
version = "0.0.0"
edition = "2021"
[workspace]
[dependencies]
...
case "$method ${endpoint#repos/example/ahx/}" in
    ...
    'POST git/trees')
        jq -e '.base_tree != null and ([.tree[].path] == ["Cargo.toml", "Cargo.lock", "fuzz/Cargo.lock", "tests/no-std/Cargo.lock", "CHANGELOG.md"])' "$input" > /dev/null
        printf 'tree-sha'
        ;;
    'POST git/commits') printf '%s' "$MOCK_TAG_COMMIT" ;;
    'POST pulls' | 'PATCH releases/42') printf 'https://github.com/example/ahx/example\n' ;;
...
if [[ $user_agent != "ahx-release ($GITHUB_REPOSITORY)" ]]; then
...
expect_failure grep -q 'POST repos/example/ahx/git/' "$MOCK_LOG"
...
grep -q 'PATCH repos/example/ahx/git/refs/tags/v0.3.0-rc.1' "$MOCK_LOG"
