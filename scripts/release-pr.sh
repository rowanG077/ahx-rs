...
if [[ -z $head ]]; then
	jq -n --arg base "$(git rev-parse 'HEAD^{tree}')" \
		...
		--rawfile fuzz_lock fuzz/Cargo.lock --rawfile consumer_lock tests/no-std/Cargo.lock \
		'{base_tree: $base, tree: [
            {path: "Cargo.toml", mode: "100644", type: "blob", content: $manifest},
            {path: "Cargo.lock", mode: "100644", type: "blob", content: $lock},
            {path: "fuzz/Cargo.lock", mode: "100644", type: "blob", content: $fuzz_lock},
            {path: "tests/no-std/Cargo.lock", mode: "100644", type: "blob", content: $consumer_lock},
...
if [[ -z $existing ]]; then
	body="$temporary/body"
	cat >"$body" <<EOF
Prepare $tag: update the Cargo version and workspace lockfiles, and finalize the changelog.
