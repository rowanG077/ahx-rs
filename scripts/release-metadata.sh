...
prepare)
	...
	# Update the crate version in each workspace lockfile. Metadata preserves
	# pinned dependencies and also fetches them on a fresh release runner.
	for manifest in Cargo.toml fuzz/Cargo.toml tests/no-std/Cargo.toml; do
